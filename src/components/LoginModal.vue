...
const handleLogin = async () => {
  ...
  try {
    ...
    const user = users.find(user => user.username === userCredentials.username && user.password === userCredentials.password)
    if (user) {
...
<template>
  <div v-if="isVisible" class="modal-content">
      ...
      <h1 class="text-blue-600 text-2xl xl:text-3xl font-extrabold m-6 text-center">
        Login
      </h1>
